...
func buggieGame() {
    print("This game has a lot of bugs, go get them! 🐛")
}

func noMusic() {
    print("This game ha no music, bro... 💃🏻")
}

func lowLevel() {
    print("This game need more than 5 levels. 🔝")
}


func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if (bugs <= bugsAllowed) && music && (levels >= minimumLevels) {
        release()
        } else {
...
func youAreReady() {
    print("Yo \(name)! Nice work, now ya ready to rooooll! 🏆")
}

func youAintReadyForNothing() {
    print("Damm \(name) you did not make it neither for the biking nor for the running... whats wrong,uh? 😐")
}

func notReadyForBike() {
    print("Hey \(name) you are not biking! You need more 🚴🏻!")
}

func notReadyForRun() {
    print("Hey \(name) you are not ready for running the duathlon! You need more 🏃‍♀️!")
}
